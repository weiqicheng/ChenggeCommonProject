package com.chengge.admin.util;
...
//        strategy.setExclude(new String[]{"sys_user_role","sys_role_menu","sys_group_ur"}); // 排除生成的表
        // 自定义实体父类
        if(type == 1){
            strategy.setSuperEntityClass("com.chengge.admin.base.DataEntity");
            // 自定义实体，公共字段
            strategy.setSuperEntityColumns(new String[] { "id","create_date","create_by","update_date","update_by","remarks","del_flag" });
        }
        if(type == 2){
            strategy.setSuperEntityClass("com.chengge.admin.base.TreeEntity");
            // 自定义实体，公共字段
